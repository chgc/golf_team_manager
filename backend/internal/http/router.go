...
func NewRouter(database *sql.DB, cfg config.Config) *gin.Engine {
	...
	apiGroup.GET("/sessions/:sessionId", apiHandlers.GetSessionByID)
	...
	apiGroup.GET("/reports/sessions/:sessionId/reservation-summary", apiHandlers.GetReservationSummary)
